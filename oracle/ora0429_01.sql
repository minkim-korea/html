...
select * from employees;
select employee_id,emp_name,salary,salary,commission_pct,salary+(salary*nvl(commission_pct,0))"실제월급" from employees;
--null +,-,*,/ 전부 null변경, nvl()->null값을 0으로 먼저 대체 
...
--null값을 ceo로 변경하시오
--manager _id 타입 : number타입  글자를 입력할수없음 변환시키고 넣어야됨 . 
select manager_id,nvl(to_char(manager_id),'ceo') from employees;

--salary, salry*12 ,salary*commission_pct,salary +(salary*commission_pct)출력
...
select * from kor_loan_status; -천만단위
--loan_jan_amt 원단위 환산해서 천단위 표시해서 출력하시오 . 
select loan_jan_amt,loan_jan_amt*10000000*1438,
to_char(loan_jan_amt*10000000,'999,999,999,999') "천단위"  from kor_loan_status; 

-- 40000 이상 역순 출력하시오 . 
select loan_jan_amt from kor_loan_status where loan_jan_amt >= 40000 order by loan_jan_amt desc;

--salary가 4000아닌 사원을 출력하시오 .     = ,  ( !=, <>,^= 다똑같)
...
select salary, employee_id ,emp_name from employees  where salary<=6000 order by salary desc;
...
--memno2 = 99.9 인테이블에서 memno =5000000,memno3 =  50.05 변경하세요 
update mem1 set memno=5000000,memno3=50.05 where memno2=99.9;
select *from mem1;

desc mem2;
select *from mem2;
--id가 aaa1111 찾아서 주민번호 0101010-2222222 kor점수 88 eng점수 95 변경 하시오 
...
select ranks from (select sno,rank() over(order by total desc) as ranks 
from stuscore);
...
update stuscore3 as a
set rank = (
select ranks from(select sno,rank() over(order by total desc) ranks from stuscore3)as b 
where a.sno = b.sno
);
