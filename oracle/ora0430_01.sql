...
--평균이 80점 이상이면서, 국어점수가 90점이상을 검색하시오 
...
--테이블복사  그대로 복사 
 ...
 ---테이블원하는 컬럼만 복사 
 --sno name kor eng math total avg rank
...
--별칭
select kor+eng as ke from stuscore3 s;
select * from stuscore3;
...
update stuscore3 a
set rank =(select ranks from (select sno,rank()over(order by total desc) ranks from stuscore3) b
where a.sno=b.sno
);
...
select avg(total) from stuscore3; --평균224.4
select avg(avg) from stuscore3; --평균 74.7999
select sum(total) from stuscore3; --전체합계

--전체평균보다 이하인 학생들에게만 전체평균값을 입력하시오 . 
update stuscore3
...
select * from employees;
--월급이  4000 이상 이면서  부서번호가 50인 사원을 검색하시오 
...
select employee_id,hire_date from employees where employee_id>=150 or hire_date<'2007/01/01';
--월급 4000 6000 7000 인 사원을 검색하시오 
select salary from employees where salary in (4000,6000,7000) ;
select salary from emeplyees where salary not in (4000,6000,7000);
select manager_id from employees where manager_id is not null;
--부서번호가 50번인 사원을 검색하시오 .
select department_id from employees where department_id=50 ; 
--부서번호가 20,30번이 아닌 사원을 검색하시오.
select department_id from employees where department_id not in (20,30);
select department_id from employees where department_id !=20 or department_id !=30; 

--월급이 4000이상 6000이하 인  사원 검색 
select * from employees 
...
--clerk 이 포함되어있는 사원 찾기 
select job_id from employees  where job_id = 'SH_CLERK' or job_id ='PU_CLERK';
select job_id from employees where job_id like '%CLERK%' ;

--이름 중에 n이 들어가있는 사원을 출력하시오 .
select * from employees where emp_name like '%n%';

--이름 중에 2번째 o이 들어가있는 사원을 검색하시오 . 
...
where hire_date >='00/01/01' and hire_date <= '07/12/31';

--member테이블에서 날짜 25년 1월 ~3월 까지  가입한 회원을 출력하시오 . 

select mdate from member order by mdate asc;

select mdate from member 
where mdate >= '2025/01/01' and mdate<='2025/03/31' order by mdate asc; 

select mdate from member 
where mdate between'25/01/01'and '25/03/31' order by mdate asc; 

--이름중에 3번째 g가 들어가는   사람을 검색하시오 . 
select * from stuscore;
select *from stuscore 
where lower(name) like '__g%'; 
--where upper(name) like '__G%';
select emp_name, salary from employees;

--salary 연봉을 , 1438 원화 , 한화 천단위 표시 -> 3번째 g 가 들어가 있는 사원만 출력 
select emp_name salary ,salary*12,salary+salary*nvl(commission_pct,0),
salary*12*1438, to_char(salary*12*1438,'999,999,999')
from employees
where lower(emp_name) like'__g%';
...
--email E또는 s 포함되어 있는 사원을 출력하시오. 
select email from employees;
select * from employees where email like '%E%' or email like '%s%' ;

select * from stuscore;
-- 2개 order by 정렬 ,1차 우선 정렬 후 같은 값일 경우 , 2차 정렬방식을 적용 2차 값이 같을때 3차 정렬 방식 적용 
